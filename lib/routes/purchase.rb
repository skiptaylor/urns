  ...
  post "/purchase/:id/confirm/?" do
    ...
    unless session[:distributor] 
      begin
        ...
        @cart = ShoppingCartItem.where(shopping_session: session[:shopping_session])
        Email.receipt(purchase.purchase_id, purchase.email, purchase.name, purchase.amount, purchase.tax, purchase.shipping_cost, purchase.total, purchase.shopping_session)

  ...
  post "/purchase/:id/confirm/?" do
    ...
    unless session[:distributor] 
      begin
        ...
        purchase.tax        = (purchase.tax_rate * purchase.total)
        
        purchase.save
        
        @cart = ShoppingCartItem.where(shopping_session: session[:shopping_session])
        Email.receipt(purchase.email, purchase.name, purchase.amount, purchase.tax, purchase.shipping_cost, session[:shopping_session])
